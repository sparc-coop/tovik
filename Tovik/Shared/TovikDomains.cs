    ...
    : BlossomAggregate<SparcDomain>(options)
{
    ...
    public async Task<SparcDomain> ClaimAsync(string domainName)
    {
        var existing = await RegisterAsync(domainName);

        if (!await existing.VerifyAsync())
                throw new Exception($"{existing.Domain} does not contain the expected Tovik script: https://tovik.app/tovik.js. Please ensure Tovik is installed correctly on this domain.");
